...
for k1 = 1:length(theta)-1
    ...
    X=[coor(1)+r*cos(theta(k1)) coor(1)+r*cos(theta(k1+1)) coor(1)+r*cos(theta(k1+1)) coor(1)+r*cos(theta(k1))];
    Y=[coor(2),coor(2),coor(2)+height,coor(2)+height];
    Z=[coor(3)+r*sin(theta(k1)) coor(3)+r*sin(theta(k1+1)) coor(3)+r*sin(theta(k1+1)) coor(3)+r*sin(theta(k1))]
    h=fill3(X,Y,Z,color);
...
X=[coor(1)+r*cos(theta(end)) coor(1)+r*cos(theta(1)) coor(1)+r*cos(theta(1)) coor(1)+r*cos(theta(end))];
Y=[coor(2),coor(2),coor(2)+height,coor(2)+height];
Z=[coor(3)+r*sin(theta(end)) coor(3)+r*sin(theta(1)) coor(3)+r*sin(theta(1)) coor(3)+r*sin(theta(end))];
h=fill3(X,Y,Z,color);
set(h,'edgealpha',0,'facealpha',facealpha)
...
% 绘制底面
fill3(coor(1)+r*cos(theta), coor(2)*ones(1,size(theta,2)), coor(3)+r*sin(theta), color) % Y坐标固定，使用X和Z坐标
% 绘制顶面
fill3(coor(1)+r*cos(theta), (coor(2)+height)*ones(1,size(theta,2)), coor(3)+r*sin(theta), color) % Y坐标加上高度，使用X和Z坐标
view(3)
